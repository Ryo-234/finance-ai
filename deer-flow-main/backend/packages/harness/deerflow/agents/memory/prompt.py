...
def _count_tokens(text: str, encoding_name: str = "cl100k_base") -> int:
    ...
    参数：
        ...
        encoding_name: 要使用的编码（默认：cl100k_base，适用于 GPT-4/3.5）。
...
def _coerce_confidence(value: Any, default: float = 0.0) -> float:
    """将置信度值强制转换为 [0, 1] 范围内的有界浮点数。

    非有限值（NaN、inf、-inf）被视为无效，并在 clamping 之前回退到默认值，
    防止它们在排名中占主导地位。假设 ``default`` 参数是一个有限值。
    """
    try:
...
def format_memory_for_injection(memory_data: dict[str, Any], max_tokens: int = 2000) -> str:
    ...
    参数：
        memory_data: 记忆数据字典。
        max_tokens: 最大 token 数量（通过 tiktoken 精确计数）。
    ...
    # 格式化 facts（按置信度排序；在 token 预算允许的情况下包含尽可能多的）
    ...
    if isinstance(facts_data, list) and facts_data:
        ...
        # 计算现有部分的 token 数量一次，然后增量计算每个事实行，
        # 以避免完整的字符串重新 token 化。
        base_text = "\n\n".join(sections)
        base_tokens = _count_tokens(base_text) if base_text else 0
        # 计算现有部分和 facts 部分之间的分隔符 token
        facts_header = "Facts:\n"
    ...
    if token_count > max_tokens:
        # 截断以符合 token 限制
        # 根据 token 比率估算要移除的字符数
        char_per_token = len(result) / token_count
        target_chars = int(max_tokens * char_per_token * 0.95)  # 95% 以留出边距
...
def format_conversation_for_update(messages: list[Any]) -> str:
    ...
    for msg in messages:
        ...
        # 从人类消息中剥离 uploaded_files 标签，以避免将
        # 临时文件路径信息持久化到长期记忆中。如果剥离后什么都没剩，
        # 则跳过该轮次（仅上传的消息）。
        if role == "human":
